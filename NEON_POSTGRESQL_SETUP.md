...
This guide explains how to set up and configure Neon PostgreSQL for your DataHarbour mobile data vending application.
...
4. Choose a project name (e.g., "DataHarbour")
...
2. **Run the schema** from `dataharbour_postgresql.sql`:

```sql
-- Copy and paste the entire content of dataharbour_postgresql.sql
...
3. Copy the entire content from `dataharbour_postgresql.sql`
...
psql "postgresql://username:password@hostname.neon.tech:5432/database?sslmode=require" -f dataharbour_postgresql.sql
```
...
Your DataHarbour application is now configured to work with Neon PostgreSQL!