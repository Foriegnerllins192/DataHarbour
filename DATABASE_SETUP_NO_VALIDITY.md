...
This guide will help you set up the MySQL database for the DataHarbour mobile data selling website using XAMPP. This version removes validity periods from all data bundles so they don't expire.
...
2. In the "Create database" field, enter: `dataharbour`
3. Select "utf8mb4_general_ci" as the collation
4. Click the "Create" button

### 4. Import the Database Schema
1. Click on the `dataharbour` database in the left sidebar
2. Click on the "Import" tab at the top
3. Click the "Choose File" button
4. Navigate to your project folder: `c:\xampp\htdocs\DATAHARBOUR\`
5. Select the `dataharbour_no_validity.sql` file
6. Make sure "Format" is set to "SQL"
7. Click the "Go" button at the bottom

### 5. Verify the Database Setup
After importing, you should see three tables in the `dataharbour` database:
- `users` - Contains user accounts (including admin)
- `data_plans` - Contains mobile data plans for different providers (without validity periods)
...
**Note:** The `validity_days` column has been removed since data bundles don't expire.
...
1. Make sure the database name matches exactly (`dataharbour`)
...
1. Drop the `dataharbour` database
